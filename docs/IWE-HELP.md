...
| Вид | Что | Примеры |
|-----|-----|---------|
| **Системы** | Программы с 4D-границами | Claude Code, Cursor, GitHub Copilot, Antigravity, Telegram-бот, MCP-серверы, WakaTime, Git, экзокортекс (файлы), Neon DB |
| **Описания** | Знания, загружаемые в системы | FPF/SPF/ZP, Pack-сущности, промпты ролей, содержимое экзокортекса |
| **Роли** | Функция, не исполнитель | Стратег (R1) ← AI-агент, Экстрактор (R2), Синхронизатор (R8), Пользователь ← Человек |
...
- Один из поддерживаемых AI-профилей: Claude Code, Cursor, GitHub Copilot или Antigravity
- Node.js v18+ и npm — только для профиля Claude Code

### Опционально
- VS Code, Cursor или другой редактор с терминалом. Claude Code — CLI; Copilot и Cursor работают через IDE; Antigravity — через собственный плагин/редактор
...
- CLAUDE.md, AGENTS.md и memory/ — настроенные под тебя
...
MCP (Model Context Protocol) — протокол, через который AI-агент подключается к базе знаний платформы. Один Gateway-сервер агрегирует все бэкенды:
...
Для Claude Code MCP подключается через https://claude.ai/settings/connectors (см. SETUP-GUIDE §1.3b). Для других профилей используй их встроенный механизм knowledge/MCP integration. Проверка: запроси «Найди документы про принципы» и убедись, что агент использует `knowledge_search`.
...
Каждая сессия в AI-агенте — три стадии:

**Открытие.** Даёшь задание → агент проверяет WP Gate (есть ли в плане недели?). Если нет — предлагает добавить. Объявляет роль, метод, оценку.

**Работа.** Агент выполняет задачу. На рубежах фиксирует знания: «Capture: [что] → [куда]».

**Закрытие.** Скажи «закрывай» → агент коммитит, пушит, обновляет память, бэкапит.
