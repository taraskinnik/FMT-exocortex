...
| **T4: Созидание (IWE)** | + AI code editor/agent, Стратег, Git, свои базы знаний | **Это руководство** |
...
- **AI-агент** — Claude Code, Cursor, GitHub Copilot или Antigravity, который знает твои цели, задачи и методологию. Помнит контекст между сессиями
...
|------|-----|-------|---------------------|
| **0** | Подготовка (Git, GitHub CLI, AI-агент) | 15-20 мин | **обязательно** |
...
> **Минимум для старта:** Этапы 0 → 1 → 2. Всё остальное подключается в любой момент — скажи агенту *«настрой календарь»* или *«подключи видеозаписи»*.
...
Если у тебя уже установлены Git, GitHub CLI и нужный AI-агент — переходи к [Этапу 1](#этап-1-установка-iwe-5-мин).
...
Node.js — среда выполнения JavaScript. Нужна только если ты выбираешь профиль Claude Code. npm — менеджер пакетов Node.js (устанавливается вместе с Node.js).
...
IWE поддерживает несколько профилей установки. Выбор делается в `setup.sh`, но подготовить инструмент лучше заранее:

| Профиль | Что установить | Как запускать | Комментарий |
|---------|----------------|---------------|-------------|
| **Claude Code** | `npm install -g @anthropic-ai/claude-code` | `claude` | Максимальная совместимость со `.claude/skills` |
| **Cursor** | Cursor IDE + optional CLI | `cursor` | Поддержка через `AGENTS.md` и `.cursor/rules/` |
| **GitHub Copilot** | VS Code + GitHub Copilot | `code` + Copilot Chat | Поддержка через `.github/copilot-instructions.md` |
| **Antigravity** | IDE/плагин Antigravity | через IDE/плагин | Базовая совместимость через `AGENTS.md` |

> `setup.sh` попросит выбрать профиль интерактивно и подстроит memory layout и инструкции под него.
...
VS Code — редактор кода с графическим интерфейсом. В нём удобно работать с Claude Code и GitHub Copilot: видишь файлы, терминал и ИИ-ассистента в одном окне. **Без IDE** придётся работать только через терминал — это возможно, но менее наглядно.

- Скачай и установи: [code.visualstudio.com](https://code.visualstudio.com/)
- Открой VS Code → нажми `Cmd+Shift+X` (macOS) или `Ctrl+Shift+X` (Windows/Linux) → установи нужное расширение (`Claude Code` или `GitHub Copilot`)
...
| Agent profile | Выбранный AI-агент | `Claude Code` / `Cursor` / `GitHub Copilot` / `Antigravity` |
| Agent command/path | Команда или путь запуска | Просто нажми Enter (определяется автоматически) |
...
2. Установит `CLAUDE.md` и `AGENTS.md` — source-of-truth и compatibility entrypoint
3. Установит `memory/` — оперативную память для выбранного агентного профиля
4. Настроит agent-specific слой (`.claude/`, `.cursor/rules/`, `.github/copilot-instructions.md`) и MCP там, где это применимо
...
Если выбран профиль Claude Code, открой Claude Code в папке экзокортекса и набери `/mcp` -- серверы должны быть в статусе Connected. Затем попроси:
> Найди документы про принципы

Claude должен использовать `knowledge_search("принципы")` и вернуть список документов из базы знаний. Для других профилей используй тот механизм MCP/knowledge connectors, который поддерживает выбранный редактор.
...
**Вариант А — через IDE (рекомендуется):**
1. Открой выбранную IDE (`VS Code`, `Cursor` или Antigravity)
2. `File → Open Folder` → выбери папку `~/IWE`
3. Открой чат/панель выбранного AI-агента

**Вариант Б — через терминал:**
```bash
cd ~/IWE
# Команда зависит от выбранного профиля: claude / cursor / code
```
...
Агент прочитает `AGENTS.md`, `CLAUDE.md` и `memory/` и проведёт тебя через:
